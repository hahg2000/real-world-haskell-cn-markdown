...
我们使用 Haskell 的 `有限载入` 语法载入 `ByteString` 模块， 像上面
`import qualified`
...
这些模块中的函数适用于单字节大小的 `Char` 值，所以他们仅适用于 `ASCII`
...
对于正则表达式不存在这个限制。正则表达式可以是 `String` 或 `ByteString`
，而不必在意输入或结果是何种类型。
...
2.  如果你在使用类 Unix 系统，查阅 `System.Posix.Files`
