...
示例程序维持两个 `Map` ：一个从用户名映射到 `PasswdEntry` ，另一个从 UID
映射到 `PasswdEntry`
...
回头看看这些代码，我们已经完成了既定目标：给 `SymbolicManip`
